  ...
  module Container
    module Wls
      ...
      class WlsConfigurer
        ...
        def configure
          ...
          unless wls_home_glob
            log_and_print("Problem with install, can't locate weblogic.jar, check captured install log output " \
                          "at #{@wls_sandbox_root}/install.log")
          end

          @wls_home = File.dirname(wls_home_glob) + '/../..'
          unless @wls_home
            log_and_print("Problem with install, check captured install log output at #{@wls_sandbox_root}/install.log")
          end
        ...
        CUSTOM_DOMAIN_EXTENSION_SCRIPT = 'customDomainExtension.py'.freeze
        ...
        def link_jars_to_domain
          ...
          system '/bin/cp', '-R', "#{@wls_buildpack_config_cache_root}/#{WLS_PRE_JARS_CACHE_DIR}", \
                 "#{@config_cache_root}/"
          system '/bin/cp', '-R', "#{@wls_buildpack_config_cache_root}/#{WLS_POST_JARS_CACHE_DIR}", \
                 "#{@config_cache_root}/"
